<template>
    <div class="card">
      <div class="card-body scrollable">
        ...
        <h3 class="mt-2" style="text-align:center"><strong>Sushiswap Assets</strong></h3>
        ...
        <table class="table">
            ...
            <tbody v-else>
                <tr v-for="row in infoUniswap.items" :key="row">
                    ...
                    <td v-if="row.total_liquidity_quote !== null">${{ numberWithCommas(row.total_liquidity_quote) }}</td>
                    <td v-if="row.volume_24h_quote !== null">${{ numberWithCommas(row.volume_24h_quote) }}</td>
                    <td v-if="row.fee_24h_quote !== null">${{ numberWithCommas(row.fee_24h_quote) }}</td>
                    <td v-if="row.swap_count_24h !== null">{{ numberWithCommas(row.swap_count_24h) }}</td>
                </tr>    
            </tbody>
